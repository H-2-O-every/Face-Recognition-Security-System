...
void FaceNetClassifier::featureMatching(cv::Mat &image, char web_name[]){
	std::string name = "Not Recognized";
    for(int i = 0; i < (m_embeddings.size()/128); i++) {
        ...
        if (minDistance <= m_knownPersonThresh) { // if it is recognized.
			...
			name= m_knownFaces[winner].className;
			// here code. by using return ??
        }
