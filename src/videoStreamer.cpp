...
VideoStreamer::VideoStreamer(int nmbrDevice, int videoWidth, int videoHeight, int frameRate, bool isCSICam) {
	if(isCSICam) {
		...
		//m_capture = new cv::VideoCapture("../testimgs/test.jpg", cv::CAP_GSTREAMER);
		...
		m_capture = new cv::VideoCapture("../testimgs/test.jpg");
		cv::Mat test_image;
		*m_capture >> test_image;
		//cv::imshow("test.jpg", test_image);
		//cv::waitKey(0);
